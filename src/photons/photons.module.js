...
class ParticleStateArray {
  ...
  allocate(particleCount) {
    ...
    for (let i = 0; i < particleCount; i++)
      this.particleStates[i] = new ParticleState();
  }
  ...
  copyState(srcIndex, destIndex) {
    if (srcIndex >= this.particleCount) {
      throw new Error(
        'ParticleStateArray::copyState() -> "srcIndex" is out of range.'
      );
    }
    if (destIndex >= this.particleCount) {
      throw new Error(
        'ParticleStateArray::copyState() -> "destIndex" is out of range.'
      );
    }
  ...
  setState(index, state) {
    if (index >= this.particleCount) {
      throw new Error(
        'ParticleStateArray::setState() -> "index" is out of range.'
      );
    }
  ...
  getState(index) {
    if (index >= this.particleCount) {
      throw new Error(
        'ParticleStateArray::getState() -> "index" is out of range.'
      );
    }
...
class ParticleSequenceGroup {
  ...
  addSequence(start, length, id) {
    if (this.hasID(id)) {
      throw new Error(
        "ParticleSequenceGroup::addParticleSequence -> Tried to add sequence with duplicate ID."
      );
    }
...
class ComponentContainer {
  ...
  getComponent(index) {
    if (index >= this.components.length) {
      throw new Error(
        'ComponentContainer::getComponent() -> "index" is out of range.'
      );
    }
...
class ParticleSystem {
  ...
  init(maximumActiveParticles) {
    if (!this.initialized) {
      ...
      if (this.particleSystemRenderer) {
        ...
        this.particleSystemRenderer.init(
          this.maximumActiveParticles,
          this.simulateInWorldSpace
        );
        this.particleSystemRenderer.setSimulateInWorldSpace(
          this.simulateInWorldSpace
        );
        this.particleStates =
      ...
      throw new Error(
        "ParticleSystem::init() -> trying to intialize more than once."
      );
    }
  }
    ...
    if (this.systemState == ParticleSystemState.Running) {
      ...
      currentTime =
        currentTime == undefined || currentTime == null
          ...
          : currentTime;
      timeDelta =
        timeDelta == undefined || timeDelta == null
          ? currentTime - this.lastUpdateTime
          : timeDelta;
      if (
        this.emitterInitialized &&
        ...
        if (this.onUpdateCallback)
          this.onUpdateCallback(this.activeParticleCount);
  ...
  getParticleStateInitializer(index) {
    if (index >= this.particleStateInitializers.length) {
      throw new Error(
        'ParticleSystem::getParticleStateInitializer() -> "index" is out of range.'
      );
    }
  ...
  getParticleStateOperator(index) {
    if (index >= this.particleStateOperators.length) {
      throw new Error(
        'ParticleSystem::getParticleStateOperator() -> "index" is out of range.'
      );
    }
  ...
  getParticleState(index) {
    if (index >= this.activeParticleCount) {
      throw new Error(
        'ParticleSystem::getParticleState() -> "index" is out of range.'
      );
    }
  ...
  setTransformInitialDirectionInWorldSpace(
    transformInitialDirectionInWorldSpace
  ) {
  ...
  updateBounds = (function () {
    ...
    return function () {
      ...
      if (this.particleSystemRenderer.calculatingBoundingSphereFromBox()) {
        this.particleStates.computeBoundingBox(
          this.boundingBox,
          positionTransform
        );
        this.particleSystemRenderer.setBoundingBox(this.boundingBox);
      } else {
        this.particleStates.computeBoundingSphere(
          this.boundingSphere,
          positionTransform
        );
  ...
  activateParticles(particleCount) {
    if (this.systemState == ParticleSystemState.Running) {
      const newActiveParticleCount = Utils.clamp(
      ...
      if (this.simulateInWorldSpace) {
        ...
        if (this.transformInitialDirectionInWorldSpace) {
          particleState.velocity
            .transformDirection(this.owner.matrixWorld)
            .multiplyScalar(vLength);
          const aLength = particleState.acceleration.length();
          particleState.acceleration
            .transformDirection(this.owner.matrixWorld)
  ...
  advanceActiveParticle(index, timeDelta) {
    if (this.systemState == ParticleSystemState.Running) {
      ...
      for (let i = 0; i < this.particleStateOperators.length; i++) {
        ...
        const stillAlive = particleStateOperator.updateState(
          particleState,
          timeDelta
        );
        ...
        if (
          !stillAlive ||
          (particleLifeTime != 0.0 && particleState.age >= particleLifeTime)
        ) {
          return false;
        }
      }
  ...
  static fromJSON(json, jsonTypeStore, threeRenderer) {
    ...
    if (json.sequences) {
      for (const sequenceJSON of json.sequences) {
        particleSystem.addParticleSequence(
          sequenceJSON.start,
          sequenceJSON.length,
          sequenceJSON.id
        );
    ...
    if (json.initializers) {
      for (const initializerJSON of json.initializers) {
        particleSystem.addParticleStateInitializer(
          initializerJSON.type.fromJSON(particleSystem, initializerJSON.params)
        );
      }
    }
    ...
    if (json.operators) {
      for (const operatorJSON of json.operators) {
        const operator = particleSystem.addParticleStateOperator(
          operatorJSON.type.fromJSON(particleSystem, operatorJSON.params)
        );
  ...
  toJSON(jsonTypeStore) {
    ...
    const sequences = particleSequenceGroup
      .getSequenceIDs()
      .map((sequenceID) => {
...
class Renderer {
  ...
  init(particleCount) {
    ...
    if (!this.initialized) {
      ...
      throw new Error(
        "Renderer::init() -> trying to intialize more than once."
      );
    }
  }
...
class ParticleStateAttributeArray extends ParticleStateArray {
    ...
    if (activeParticleCount > 0) {
      if (this.instanced) {
        ...
        this.geometry.setDrawRange(
          0,
          this.verticesPerParticle * activeParticleCount
        );
      }
    } else {
      if (this.instanced) {
  ...
  flushParticleStateToBuffers(index) {
    if (index >= this.particleCount) {
      throw new Error(
        'ParticleStateAttributeArray::flushParticleStateToBuffers() -> "index" is out of range.'
      );
    }
    ...
    for (let i = 0; i < this.verticesPerParticle; i++) {
      ...
      this.sequenceElements.setXYZW(
        offset + i,
        particleState.sequenceElement.x,
        particleState.sequenceElement.y,
        particleState.sequenceElement.z,
        particleState.sequenceElement.w
      );
      ...
      this.positions.setXYZ(
        ...
        particleState.position.z
      );
      this.positions.needsUpdate = true;

      this.velocities.setXYZ(
        offset + i,
        particleState.velocity.x,
        particleState.velocity.y,
        particleState.velocity.z
      );
      this.velocities.needsUpdate = true;

      this.accelerations.setXYZ(
        offset + i,
        particleState.acceleration.x,
        particleState.acceleration.y,
        particleState.acceleration.z
      );
      this.accelerations.needsUpdate = true;

      this.normals.setXYZ(
        offset + i,
        particleState.normal.x,
        particleState.normal.y,
        particleState.normal.z
      );
      ...
      this.colors.setXYZ(
        offset + i,
        particleState.color.r,
        particleState.color.g,
        particleState.color.b
      );
      ...
      this.initialSizes.setXY(
        offset + i,
        particleState.initialSize.x,
        particleState.initialSize.y
      );
  ...
  setState(index, state) {
    if (index >= this.particleCount) {
      throw new Error(
        'ParticleStateAttributeArray::setState() -> "index" is out of range.'
      );
    }
  ...
  allocate(particleCount) {
    ...
    const createAttributeBuffer = (bufferArray, componentCount) => {
      if (this.instanced) {
        return new THREE$1.InstancedBufferAttribute(
          bufferArray,
          componentCount
        );
      } else {
...
class AnimatedSpriteRenderer extends Renderer {
  constructor(
    instanced,
    atlas,
    interpolateAtlasFrames = false,
    blending = THREE$1.NormalBlending,
    calculateBoundingSphereFromBox = true,
    renderOrder
  ) {
    super();
    this.instanced = instanced;
  ...
  setSimulateInWorldSpace(simulateInWorldSpace) {
    ...
    if (this.material) {
      this.material.uniforms.simulateInWorldSpace.value = simulateInWorldSpace
        ? 1
        : 0;
  ...
  init(particleCount) {
    if (super.init(particleCount)) {
      ...
      this.mesh = new THREE$1.Mesh(
        this.particleStateArray.getGeometry(),
        this.material
      );
  ...
  createMaterial(
    ...
    if (this.atlas) {
      for (let i = 0; i < this.atlas.getFrameSetCount(); i++) {
        ...
        shaderAtlas[i] = new THREE$1.Vector4(
          frameSet.x,
          frameSet.y,
          frameSet.width,
          frameSet.height
    ...
    vertexShader =
      vertexShader ||
      AnimatedSpriteRenderer.Shader.getVertexShader(useLogarithmicDepth);
    fragmentShader =
      fragmentShader ||
      AnimatedSpriteRenderer.Shader.getFragmentShader(
        useWebGL2,
        useLogarithmicDepth
      );
  ...
  static Shader = {
    ...
    getVertexShader: function (useLogarithmicDepth) {
      let shader = ["#include <common>", this.VertexVars].join("\n");
  ...
  static fromJSON(params) {
    ...
    const atlasTexture = new THREE$1.TextureLoader().load(
      atlasJSON.texturePath
    );
      ...
      if (textureSource) {
        ...
        if (textureData) {
          ...
          if (baseURISubStrLoc >= 0) {
            return currentSrc.substr(
              baseURI.length,
              currentSrc.length - baseURI.length
            );
          } else {
            return currentSrc;
          }
...
class ParticleEmitter {
  ...
  internalUpdate(timeDelta) {
    ...
    return (
      this.age >= this.emissionRelativeStartTime &&
...
class ConstantParticleEmitter extends ContinuousParticleEmitter {
  ...
  update(timeDelta) {
    if (this.internalUpdate(timeDelta)) {
      const toEmitThisFrame =
        this.emissionRate * this.timeActive - this.emitCount;
...
class RandomGenerator extends Generator {
  ...
  generate(out) {
    ...
    switch (this.outTypeID) {
      ...
      case BuiltinType.Vector2:
        out.set(
          this.generateForElement(uniformRange, "x"),
  ...
  clone() {
    const clone = new RandomGenerator(
      this.outType,
      this.range,
      this.offset,
      this.uniformRange,
  ...
  static getParameterJSON(param) {
    if (
      param instanceof THREE$1.Vector2 ||
      param instanceof THREE$1.Vector3 ||
      param instanceof THREE$1.Vector4 ||
      param instanceof THREE$1.Color
    )
  ...
  static fromJSON(params) {
    return new RandomGenerator(
      ...
      params.uniformRange || 0.0,
      params.uniformOffset || 0.0,
      params.normalize
    );
  }
...
class RotationalSpeedInitializer extends ParticleStateInitializer {
  ...
  static fromJSON(particleSystem, params) {
    return new RotationalSpeedInitializer(
      params.range,
      params.offset,
      params.uniformRange,
      params.uniformOffset,
...
class BoxPositionInitializer extends ParticleStateInitializer {
  ...
  static fromJSON(particleSystem, params) {
    return new BoxPositionInitializer(
      new THREE$1.Vector3().fromArray(params.range),
...
class RandomVelocityInitializer extends ParticleStateInitializer {
  constructor(
    ...
    super();
    this.directionGenerator = new RandomGenerator(
      THREE$1.Vector3,
      directionRange,
      directionOffset,
      0.0,
    ...
    this.speedGenerator = new RandomGenerator(
      0,
      speedRange,
      speedOffset,
      0.0,
  ...
  static fromJSON(particleSystem, params) {
    return new RandomVelocityInitializer(
      new THREE$1.Vector3().fromArray(params.range),
      new THREE$1.Vector3().fromArray(params.offset),
      params.speedRange,
      params.speedOffset
...
class SequenceInitializer extends ParticleStateInitializer {
  ...
  static fromJSON(particleSystem, params) {
    return new SequenceInitializer(
      particleSystem.getParticleSequences(),
      params.reverse
    );
  }
...
class SequenceOperator extends ParticleStateOperator {
  ...
  updateState(state, timeDelta) {
    const sequenceElement = state.sequenceElement;
    const activeSequence = this.particleSequences.getSequence(
      sequenceElement.y
    );
  ...
  *[Symbol.iterator]() {
  ...
  getElement(index) {
    if (index >= this.getElementCount()) {
      throw new Error(
        'ContinuousArray::getElement() -> "index" is out of bounds.'
      );
    }
  ...
  getTValue(index) {
    if (index >= this.getElementCount()) {
      throw new Error(
        'ContinuousArray::getTValue() -> "index" is out of bounds.'
      );
    }
  ...
  getInterpolatorForTypeID = (function () {
    ...
    const getVectorBasedInterpolator = (upperVector) => {
      return (tValue, elements, tValues, out) => {
        ContinuousArray.getInterpolationValuesForTValue(
          tValues,
          tValue,
          iResult
        );
        upperVector
          .copy(elements[iResult.upperIndex])
          .multiplyScalar(iResult.localT);
        out
          .copy(elements[iResult.lowerIndex])
          .multiplyScalar(1.0 - iResult.localT)
          .add(upperVector);
      };
    };
      ...
      switch (typeID) {
        case BuiltinType.Default:
          return (tValue, elements, tValues) => {
            ...
            return (
              (1.0 - iResult.localT) * elements[iResult.lowerIndex] +
...
class InterpolatorOperator extends ParticleStateOperator {
  ...
  addElementsFromElementClassAndParameters(
    ElementClass,
    elementParametersTValuePairs
  ) {
...
class OpacityInterpolatorOperator extends InterpolatorOperator {
  ...
  updateState(state) {
    if (this.relativeToInitialValue) {
      state.alpha =
        state.initialAlpha * this.getInterpolatedValue(state, state.alpha);
    } else {
...
class SizeInterpolatorOperator extends InterpolatorOperator {
  ...
  addElementsFromParameters(elementParametersTValuePairs) {
    super.addElementsFromElementClassAndParameters(
      THREE$1.Vector2,
      elementParametersTValuePairs
    );
  }
  ...
  updateState = (function () {
    ...
    return function (state) {
      ...
      if (this.relativeToInitialValue) {
        state.size.set(
          state.initialSize.x * tempSize.x,
...
class ColorInterpolatorOperator extends InterpolatorOperator {
  ...
  updateState = (function () {
    ...
    return function (state) {
      ...
      if (this.relativeToInitialValue) {
        state.color.setRGB(
          state.initialColor.r * tempColor.r,
...
class SphereRandomGenerator extends Generator {
  ...
  clone() {
    const clone = new SphereRandomGenerator(
      this.rangeTheta,
      this.offsetTheta,
      this.rangePhi,
      this.offsetPhi,
  ...
  static fromJSON(params) {
    return new SphereRandomGenerator(
      params.rangeTheta,
      params.offsetTheta,
      params.rangePhi,
      params.offsetPhi,
...
class FlickerLight extends Component {
  ...
  update(time) {
    const elapsedTimeSinceLastIntensityFlicker =
      time - this.lastIntensityFlickerTime;
    const flickerIntensityIntervalsPerSecond = 8.0;
    const flickerIntensityIntervalLength =
      1.0 / flickerIntensityIntervalsPerSecond;
    ...
    if (elapsedTimeSinceLastIntensityFlicker > flickerIntensityIntervalLength) {
      ...
      const intensityDiff =
        (Math.random() - 0.5) *
        2.0 *
        perUpdateIntervalIntensityFluxRange *
        flickerIntensityIntervalLength;
      ...
      this.nextIntensityAdjuster = Utils.clamp(
        intensityAdjuster,
        intensityFactorRangeLowerBound,
        intensityFactorRangeUpperBound
      );
    } else {
      const elapsedFlickerIntensityT =
        elapsedTimeSinceLastIntensityFlicker / flickerIntensityIntervalLength;
      const intensityAdjuster =
        (1.0 - elapsedFlickerIntensityT) * this.lastIntensityAdjuster +
        elapsedFlickerIntensityT * this.nextIntensityAdjuster;
      this.light.intensity = intensityAdjuster * this.intensity;
    }

    const elapsedTimeSinceLastPositionFlicker =
      time - this.lastPositionFlickerTime;
    const flickerPositionIntervalsPerSecond = 16.0;
    const flickerPositionIntervalLength =
      1.0 / flickerPositionIntervalsPerSecond;

    if (elapsedTimeSinceLastPositionFlicker > flickerPositionIntervalLength) {
      this.lastPositionFlickerTime = time;

      const deltaTime = time - this.lastUpdateTime;
      this.positionAdjuster.set(
        Math.random() - 0.5,
        Math.random() - 0.5,
        Math.random() - 0.5
      );
...
class JSONTypeStore {
  ...
  addTypeToNamespace(namespace, typeName, type) {
    if (!this.jsonTypes[namespace]) {
      throw new Error(
        "JSONTypeStore::addTypeToNamespace() -> namespace does not exist"
      );
    }
    if (this.jsonTypes[namespace][typeName]) {
      throw new Error(
        "JSONTypeStore::addTypeToNamespace() -> typeName already exists"
      );
    }
  ...
  addNamespace(namespace, namespaceObject) {
    if (this.jsonTypes[namespace]) {
      throw new Error(
        "JSONTypeStore::addNamespace() -> namespace already exists"
      );
    }
    this.jsonTypes[namespace] = namespaceObject;
    for (const typeName in namespaceObject) {
      if (
        !namespaceObject.hasOwnProperty ||
        namespaceObject.hasOwnProperty(typeName)
      ) {
